...
class TransactionCRUD:
  ...
  async def get_user_transactions(
    ...
    try:
      ...
      logger.info(f"사용자 거래 목록 조회 완료: user_id={user_id}, 결과 수={len(transactions)}")
      return transactions
  ...
  async def get_stock_holdings_summary(
    ...
    try:
      ...
      logger.info(f"종목별 broker 보유현황 조회 완료: user_id={user_id}, stock_symbol={stock_symbol}, 결과 수={len(lots)}")
      return lots
  ...
  async def get_realized_profits_db(
    ...
    try:
      ...
      logger.info(f"실현손익 Raw 데이터 조회 완료: user_id={user_id}, 결과 수={len(results)}")
      return results
  ...
  async def get_realized_profits_metadata(
    ...
    try:
      ...
      broker_query = (
        ...
        .join(Transaction, Broker.id == Transaction.broker_id)
        .filter(
          and_(
            Transaction.user_id == user_id,
            Transaction.transaction_type == 'SELL',
      ...
      logger.info(f"실현손익 메타데이터 조회 완료: user_id={user_id}, 종목 수={len(stocks)}, 증권사 수={len(brokers)}")
      return {
        "stocks": stocks,
        "brokers": brokers
      }
