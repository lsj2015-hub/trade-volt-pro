...
class HoldingCRUD(BaseCRUD[Holding]):
  ...
  async def get_or_create_holding(
    ...
    holding = await self._get_single_result(
      db, query, f"보유 정보 조회 실패: user_id={user_id}, stock_id={stock_id}, broker_id={broker_id}"
    )
  ...
  async def update_holding_for_buy(
    ...
    try:
      ...
      logger.info(f"매수 후 보유 정보 업데이트 완료: user_id={holding.user_id}, stock_id={holding.stock_id}, broker_id={holding.broker_id}")
      return holding
  ...
  async def update_holding_for_sell(
    ...
    try:
      ...
      logger.info(f"매도 후 보유 정보 업데이트 완료: user_id={holding.user_id}, stock_id={holding.stock_id}, broker_id={holding.broker_id}")
      return holding
  ...
  async def get_active_holdings(
    ...
    """사용자의 활성 보유 종목 조회 (관련 데이터 포함)"""
    query = (
      select(Holding)
      .options(
        joinedload(Holding.stock),
        joinedload(Holding.broker)
      )
      .filter(
        and_(
          Holding.user_id == user_id,
      ...
      .order_by(desc(Holding.last_transaction_date))
    )
    
    return await self._get_multiple_results(
      db, query, f"활성 보유 종목 조회 실패: user_id={user_id}"
    )
  ...
  async def get_user_portfolio_by_stocks(
    ...
    query = (
      ...
      .order_by(desc(func.max(Holding.last_transaction_date)))
    )
    
    result = await self._execute_query(
      db, query, f"종목별 포트폴리오 요약 조회 실패: user_id={user_id}"
    )
  ...
  async def get_stock_holdings_by_brokers(
    ...
    result = await self._execute_query(
      db, query, f"종목별 상세 보유현황 조회 실패: user_id={user_id}, stock_id={stock_id}"
    )
  ...
  async def get_user_portfolio_overview(
    ...
    result = await self._execute_query(
      db, query, f"전체 포트폴리오 통계 조회 실패: user_id={user_id}"
    )
