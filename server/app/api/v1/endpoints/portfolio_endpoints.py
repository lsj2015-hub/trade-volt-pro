...
from app.schemas.common_schemas import ( 
  CompletePortfolioResponse, RealizedProfitListResponse, PortfolioOverviewResponse, PortfolioStocksResponse, PortfolioStockItem
)
...
async def get_portfolio_overview(
  ...
  try:
    ...
    return PortfolioOverviewResponse(
      success=True,
      data=overview  # ✅ overview는 이미 dict이므로 PortfolioOverviewData로 자동 변환
    )
...
@router.get("/stocks", response_model=PortfolioStocksResponse)
async def get_portfolio_stocks(
  ...
  try:
    ...
    # Dict를 PortfolioStockItem으로 변환
    portfolio_items = [
      PortfolioStockItem(**item) for item in portfolio
    ]
    
    return PortfolioStocksResponse(
      success=True,
...
async def get_stock_detail_by_brokers(
  ...
  try:
    ...
    # Service Layer 활용 (개선된 CRUD 내부적으로 사용)
